...
class VergunningControleControllerTest extends ApiTestCase
{
    ...
    public function testGetByMarktAndDate()
    {
        $vergunningControleRepository = $this->entityManager->getRepository(VergunningControle::class);

        $vergunningControle = $vergunningControleRepository->findOneBy([], ['id' => 'DESC']);
        $marktId = $vergunningControle->getDagvergunning()->getMarkt()->getId();
        $date = $vergunningControle->getDagvergunning()->getDag()->format('Y-m-d');

        $response = $this->client->get('/api/1.1.0/controle/'.$marktId.'/'.$date, [
            'headers' => $this->headers,
        ]);

        $responseData = json_decode((string) $response->getBody(), true);
        $vergunningControle = reset($responseData);
        ...
        foreach ($expectedKeys as $expectedKey) {
            $this->assertArrayHasKey($expectedKey, $vergunningControle);
        }
    }

    public function testFlexPost()
    {
        $koopmanRepository = $this->entityManager
            ->getRepository(Koopman::class);
        $marktRepository = $this->entityManager
        ...
        $response = $this->client->post('/api/1.1.0/flex/controle/', [
            'headers' => $this->headers,
            'body' => json_encode($dataVergunningControle),
        ]);
